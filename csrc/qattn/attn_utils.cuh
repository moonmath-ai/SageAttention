...
#pragma unroll
      for (uint32_t k = 0; k < 8; k++)
      {
        ...
        else if constexpr (std::is_same<DTypeQKAccum, int32_t>::value)
        {
          RS[fq][fk][k] = (out_of_boundary ? -5000000 : RS[fq][fk][k]);
        }
      }
    }
  }
...
__device__ __forceinline__ void update_mdo_int(int32_t RS[][num_tiles_k][8], float RS_f32[][num_tiles_k][8], DTypeSVAccum RO[][num_tiles_v][8], int32_t m[][2], float d[][2], const float &sm_scale)
{
...
#pragma unroll
    for (uint32_t k = 0; k < 2; k++) {
      int32_t m_prev = m[fq][k];
      int32_t m_temp = -5000000;
#pragma unroll
      for (uint32_t fk = 0; fk < num_tiles_k; fk++) {
        int32_t m_local = max(max(RS[fq][fk][k * 2 + 0], RS[fq][fk][k * 2 + 1]),
